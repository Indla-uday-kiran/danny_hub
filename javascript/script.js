...
// State Variables (unchanged except for wakeLock addition)
let originalSongs = [
...
let wakeLock = null; // Added for Wake Lock API

// Utility Functions (unchanged except for safePlay and new wake lock functions)
...
// Updated loadSong with Media Session
                ...
                if (nextIndex !== index && nextIndex < songs.length) {
                    ...
                    nextAudio.play()
                        .then(() => {
                            ...
                            console.log("Next song buffered:", songs[nextIndex].title);
                        })
...
// New Media Session Update Function
function updateMediaSession() {
    if ("mediaSession" in navigator) {
        navigator.mediaSession.metadata = new MediaMetadata({
            ...
            artwork: [{ src: songs[currentSong].cover || "images/default-cover.jpg", sizes: "500x500", type: "image/jpeg" }]
        });
...
// Event Listeners (updated with Wake Lock)
...
playBtn?.addEventListener("click", async () => {
    await requestWakeLock(); // Request Wake Lock on play
    audio.play().then(() => {
        updatePlayPauseUI(true);
...
pauseBtn?.addEventListener("click", async () => {
    audio.pause();
    updatePlayPauseUI(false);
    await releaseWakeLock(); // Release Wake Lock on pause
...
audio?.addEventListener("ended", async () => {
    ...
    const playNext = () => {
        console.log("Advancing to next song, index:", currentSong);
        loadSong(currentSong)
            ...
            .catch(e => {
                ...
                if (songs.length > 1 && skipCount < maxSkips) {
                    ...
                    showToast("Failed to load next song");
                    updatePlayPauseUI(false);
                }
            });
    };
...
// Voice Activation Functions (unchanged except for context)
        ...
        switch (matchedKey) {
            ...
            case "cancelSleep":
                if (sleepTimerInterval) {
                    ...
                    sleepTimer.value 
                    "0";
                    respond("Sleep Timer Cancelled");
                } else {
                    respond("No Sleep Timer Active");
                }
                break;
            default:
...
function handlePlaySongCommand(action) {
    const songName = action.replace(/play song|play/i, "").trim();
    if (!songName) {
    ...
    if (song) {
        ...
        loadSong(index).then(() => {
            audio.play().then(() => {
                ...
                if (isVoiceFeedbackEnabled) speak(`Playing ${song.title}`);
            }).catch(e => {
                console.error("Play error:", e);
                showToast("Playback blocked. Please interact with the page.");
            });
            ...
            showToast(`Error loading "${song.title}"`);
        });
    } else {
...
document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "hidden" && !audio.paused) {
        console.log("Page hidden, audio playing");
        if ("mediaSession" in navigator) {
            navigator.mediaSession.playbackState = "playing";
        }
...
// Service Worker Registration (already included earlier, repeated for completeness)
if ("serviceWorker" in navigator) {
    window.addEventListener("load", () => {
        navigator.serviceWorker.register("/service-worker.js", { scope: "/" })
            .then(registration => {
